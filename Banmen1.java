...
public class Banmen1 {
	...
	public  void play() {
		...
		if (board[num] == 0) {
			...
			System.out.println("入力エラー、●の番");
			
			continue;//不满足阻断当前循环，但是循环还会继续下去 只是下面i++不实行 这时的i还是等于当前被阻断时的值 一直不满足就一直循环
		}
		//找出最左和最右边的黑棋
		if (i != 0) {
		play1(i, num);
		...
		if (i % 2 == 1 ) {
			...
			if (board[num] == 0) {
				...
				System.out.println("入力エラー、○の番");
				continue;//同上 不满足阻断当前循环，但是循环还会继续下去 只是下面i++不实行 这时的i还是等于当前被阻断时的值 一直不满足就一直循环
			}
			if (i != 1) {
				play1(i, num);
			}
	...
	public void play1(int i,int num) {
		...
		//System.out.println("右= " + migi);
		num2 = migi - hidari;//两棋的 间距
		//System.out.println("区間＝" + num2);
